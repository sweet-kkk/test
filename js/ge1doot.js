...
var ge1doot = ge1doot || {
	...
	//callback：执行初始化函数
	//data：图片数据的信息
	loadJS: function (url, callback, data) {  // 页面加载的过程中立即执行此段代码
		if (typeof url == "string") url = [url];
		//alert(JSON.stringify(data.imgdata))
		// alert(callback)
		var load = function (src) {
			...
			var script = document.createElement("script");
				...
				script.src = src;
				document.getElementsByTagName("head")[0].appendChild(script);  //获取HTML文件中head标签里的所有内容
		}
		for (var i = 0, n = url.length; i < n; i++) 
			load(url[i]);  // for循环，通过url载入
		//return JSON.stringify(data.imgdata)
	}
}

// ===== html/canvas container =====  得到画板容器
...
// ==== unified touch events handler ====  统一触摸事件处理程序
